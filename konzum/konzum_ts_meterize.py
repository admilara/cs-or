# -*- coding: utf-8 -*-
"""
Created on Tue Dec 10 23:31:23 2024
...
konzum_path = "D:\\3_RADNO\\_HOPS\\2_ZAKUCAC_CS_OP\\HOPS\\mjerenja\\konzum\\TS_MET_TR1_TR2_20241204_0910-1710.csv"
...
for i in range(int(len(column_names)/2)):
    fig = graf(split_dfs[0], column_names[i], column_names[i+4], values[i], 
               values[i+4], units[i], colors[i], colors2[i], otocni_rad[0])
    if i == 0:
...
for i in range(8,12,2):
    fig = graf(split_dfs[0], column_names[i], column_names[i+1], values[i],
               values[i+1], units[int(i/2)], colors[int(i/2)], colors2[int(i/2)], otocni_rad[0])
    html_list[int(i/2)] = pio.to_html(fig, full_html=False, include_plotlyjs=False)
...
<html>
<head>
    <title>konzum-meterize-OR-A</title>
...
for i in range(int(len(column_names)/2)):
    fig = graf(split_dfs[1], column_names[i], column_names[i+4], values[i], 
               values[i+4], units[i], colors[i], colors2[i], otocni_rad[1])
    if i == 0:
        html_list[i] = pio.to_html(fig, full_html=False, include_plotlyjs="cdn")
    else:
        html_list[i] = pio.to_html(fig, full_html=False, include_plotlyjs=False)
...
for i in range(8,12,2):
    fig = graf(split_dfs[1], column_names[i], column_names[i+1], values[i],
               values[i+1], units[int(i/2)], colors[int(i/2)], colors2[int(i/2)], otocni_rad[1])
    html_list[int(i/2)] = pio.to_html(fig, full_html=False, include_plotlyjs=False)
...
<html>
<head>
    <title>konzum-meterize-OR-D</title>
    <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
</head>
<body>
...
for i in range(int(len(column_names)/2)):
    fig = graf(split_dfs[2], column_names[i], column_names[i+4], values[i], 
               values[i+4], units[i], colors[i], colors2[i], otocni_rad[2])
    if i == 0:
        html_list[i] = pio.to_html(fig, full_html=False, include_plotlyjs="cdn")
    else:
        html_list[i] = pio.to_html(fig, full_html=False, include_plotlyjs=False)
...
for i in range(8,12,2):
    fig = graf(split_dfs[2], column_names[i], column_names[i+1], values[i],
               values[i+1], units[int(i/2)], colors[int(i/2)], colors2[int(i/2)], otocni_rad[2])
    html_list[int(i/2)] = pio.to_html(fig, full_html=False, include_plotlyjs=False)
...
<html>
<head>
    <title>konzum-meterize-OR-B</title>
    <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
</head>
<body>
...
for i in range(int(len(column_names)/2)):
    fig = graf(split_dfs[3], column_names[i], column_names[i+4], values[i], 
               values[i+4], units[i], colors[i], colors2[i], otocni_rad[3])
    if i == 0:
        html_list[i] = pio.to_html(fig, full_html=False, include_plotlyjs="cdn")
    else:
        html_list[i] = pio.to_html(fig, full_html=False, include_plotlyjs=False)
...
for i in range(8,12,2):
    fig = graf(split_dfs[3], column_names[i], column_names[i+1], values[i],
               values[i+1], units[int(i/2)], colors[int(i/2)], colors2[int(i/2)], otocni_rad[3])
    html_list[int(i/2)] = pio.to_html(fig, full_html=False, include_plotlyjs=False)
...
<html>
<head>
    <title>konzum-meterize-OR-C</title>
    <script src="https://cdn.plot.ly/plotly-latest.min.js"></script>
</head>
<body>
