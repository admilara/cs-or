# -*- coding: utf-8 -*-
"""
Created on Fri Apr  4 10:40:48 2025
...
def draw_grouped(df, signal_keys, title, signal_meta, timestamp):
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    color_cycle = ['blue', 'red', 'green', 'orange']
    
    for idx, signal_key in enumerate(signal_keys):
        if signal_key not in df.columns:
            continue
        is_secondary = idx % 2 == 1  # Alternate sides
        trace = go.Scatter(
            x=df['Datetime'],
            y=df[signal_key],
            name=signal_meta[signal_key]['label'] + " [" + signal_meta[signal_key]['unit'] + "]",
    ...
    fig.update_layout(
        ...
        template="plotly_white",
        legend_title="Legenda",
        xaxis=dict(title="Vrijeme", showgrid=True),
    )

    for point in ['start', 'end']:
        if df["Datetime"].min() <= timestamp[point] <= df["Datetime"].max():
            fig.add_vline(
                x=timestamp[point],
                line_width=1,
                line_dash="dash",
            ...
            fig.add_annotation(
                ...
                y=1,
                yref="paper",
                text=f"{'ODVAJANJE' if point == 'start' else 'SINKRONIZACIJA'}: {timestamp[point]}",
                showarrow=True,
                arrowhead=2,
                ax=20,
                ay=-40,
...
grouped_signals = {
    'Napon generatora i referenca napona': ['VGACT', 'VGACTINV', 'VGREF'],
    'Napon i struja uzbude': ['IFACT', 'UFACT'],
    'Impulsi povećanja i smanjenja reference napona': ['VRINC', 'VRDEC', 'REFRINC1', 'REFRDEC1']
}
...
#TODO - DRAW FIGS
    ...
    fig_list = []
    plotted = set()
    for title, keys in grouped_signals.items():
        existing_keys = [key for key in keys if key in table_df.columns]
        if existing_keys:
            fig = draw_grouped(table_df, existing_keys, title, signali, timestamps[file_set[1]])
            fig_list.append(fig)
            plotted.update(existing_keys)

    # Then plot remaining signals one by one
    for column in column_list:
